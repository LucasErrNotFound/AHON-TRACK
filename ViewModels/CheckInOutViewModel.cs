...
public partial class CheckInOutViewModel : ViewModelBase, INotifyPropertyChanged, INavigable
{
    ...
    private async Task OnSubmitDeleteSingleDataItem(WalkInPerson walkInPerson)
    {
        try
        {
            if (_checkInOutService != null)
            {
                ...
                if (success)
                {
                    ...
                    OriginalWalkInData.Remove(walkInPerson);
                    UpdateWalkInCounts();

                    _toastManager.CreateToast($"Deleted {walkInPerson.FirstName} {walkInPerson.LastName}")
                        .WithContent("Walk-in record deleted successfully!")
                        .DismissOnClick()
                        .WithDelay(6)
                        .ShowSuccess();
                }
                else
                {
                    _toastManager.CreateToast("Delete Failed")
                        .WithContent("Failed to delete walk-in record")
                        .ShowError();
                }
            }
        }
        ...
        {
            Debug.WriteLine($"Error deleting walk-in: {ex.Message}");
            _toastManager.CreateToast("Delete Error")
                .WithContent($"Error deleting record: {ex.Message}")
                .ShowError();
        }
    }
    ...
    private async Task OnSubmitDeleteSingleDataItem(MemberPerson memberPerson)
    {
        try
        {
            if (_checkInOutService != null)
            {
                ...
                if (success)
                {
                    ...
                    OriginalMemberData.Remove(memberPerson);
                    UpdateMemberCounts();

                    _toastManager.CreateToast($"Deleted {memberPerson.FirstName} {memberPerson.LastName}")
                        .WithContent("Member check-in record deleted successfully!")
                        .DismissOnClick()
                        .WithDelay(6)
                        .ShowSuccess();
                }
                ...
                {
                    _toastManager.CreateToast("Delete Failed")
                        .WithContent("Failed to delete member check-in record")
                        .ShowError();
                }
            }
        }
        ...
        {
            Debug.WriteLine($"Error deleting member check-in: {ex.Message}");
            _toastManager.CreateToast("Delete Error")
                .WithContent($"Error deleting record: {ex.Message}")
                .ShowError();
        }
